...
public class NeuralNet_1 implements  NeuralNetInterface {
    ...
    public NeuralNet_1(double[][] inputs,
    ...
    /**
     * training all input data {{0,0},{0,1},{1,0},{1,1}}
     */
    public double trainAll() {
        ...
        for (int i = 0; i < inputs.length; i++) {
            ...
            for (int j = 0; j < numOutputs; j++) {
                error += Math.pow(output[j] - expectedOutputs[i][j], 2) / 2;
            }
     ...
     */
    public int converge(double targetError) {
        int epoch = 0;
        epochErrors = new ArrayList<>();
        double epochError = 1;
        while (epochError > targetError) {
            epochError = trainAll();
            epoch++;
            System.out.println("Error at epoch " + epoch + " is " + epochError);
        }
        System.out.println("Target error reached at " + epoch + " epochs" );

        XYLineChart_AWT chart = new XYLineChart_AWT("CPEN_502_a1",
                (binary ? "Binary, " : "Bipolar, ") + "momentum = " + momentum, epochErrors);
        chart.pack( );
